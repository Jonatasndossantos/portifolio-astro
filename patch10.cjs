...
// `group-focus-within:hidden` might be hiding it initially if the group is focused when playwright screenshots.
...
pagesSearchContent = pagesSearchContent.replace(
    ...
    '<div class="absolute right-4 top-1/2 -translate-y-1/2 px-2 py-1 bg-base-200 rounded-lg text-[10px] font-bold opacity-40 group-hover:opacity-60 transition-opacity pointer-events-none uppercase tracking-tighter flex items-center gap-1 group-focus-within:hidden">\n                        <kbd class="kbd kbd-xs bg-base-100 font-sans border-base-content/20">/</kbd>\n                    </div>'
);
